const fs = require('fs')
const { logger, logError } = require('../modules/logging')
...
class ConfigCache {
	...
	put(key, value) {
		if (!(key instanceof Array)) {
			...
			for (const k of key.slice(0, -1)) {
				if (currCache[this._keyTransform(k)] === undefined) {
					currCache[this._keyTransform(k)] = new Object()
				}
				currCache = currCache[this._keyTransform(k)]
			}
	...
	remove(key) {
		if (!(key instanceof Array)) {
			delete this._cache[this._keyTransform(key)]
		}
		else {
			let currCache = this._cache
			// descend through nested keys, initializing empty ones along the way as necessary to build the tree
			...
			try {
				...
				delete currCache[this._keyTransform(key.at(-1))]
				...
				if (Object.keys(currCache).length === 0) {
					for (let i = 0; i < keys_to_check.length; i++) {
						...
						for (const k of keys_to_check) {
							if (!currCache[this._keyTransform(k)] || Object.keys(currCache[this._keyTransform(k)]).length === 0) {
								delete currCache[this._keyTransform(k)]
							}
							else {
								currCache = currCache[this._keyTransform(k)]
							}
